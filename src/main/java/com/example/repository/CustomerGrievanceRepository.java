package com.example.repository;

import java.util.List;

import org.springframework.data.jpa.repository.EntityGraph;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.stereotype.Repository;

import com.example.model.CustomerGrievance;

@Repository
public interface CustomerGrievanceRepository extends JpaRepository<CustomerGrievance, Long> {
	@Query(value = "select * from customer_grievance where resolution_status = 'PENDING'", nativeQuery = true)
	public List<CustomerGrievance> getAllPendingCustomerGrievances();
	
	@EntityGraph(attributePaths = {"customerCardAccount.customerProfile"})
    List<CustomerGrievance> findAll();

}
